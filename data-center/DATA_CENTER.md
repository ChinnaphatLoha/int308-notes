...
**TIA** stands for the *Telecommunications Industry Association*. Their standard, **TIA-942**, is essentially the **master blueprint** for designing a data center. It covers:
...
**ASHRAE** (the *American Society of Heating, Refrigerating and Air-Conditioning Engineers*) defines the **exact temperature and humidity** that servers need.
...
> Think of ASHRAE as the doctor saying: "Your patient's body temperature should be between 36°C and 37°C. It *can* survive at 35°C or 38°C, but that's risky."

#### NFPA — Fire Safety

**NFPA** stands for the *National Fire Protection Association*. They write the rules for preventing and handling fires:
...
- **Resiliency** — The system can recover quickly when something *does* fail.
...
> *"What happens in the few seconds between the power going out and the generator starting up?"*
...
Data centers don't use regular air conditioners. They use **Precision Air Conditioning** — specialized units designed for server rooms. Regular AC is designed to keep *people* comfortable (it removes a lot of moisture). PAC is designed to keep *servers* comfortable (it focuses almost entirely on removing *heat*).
...
- **In-Row Cooling** — Small cooling units placed *directly between server racks* for more targeted cooling. Instead of cooling the entire room, they cool exactly where the heat is.
...
> **Analogy:** Regular smoke detectors wait for smoke to drift up to them. VESDA actively *sniffs* the air everywhere in the room, all the time.
...
Think of data center security like layers of an onion — an attacker would have to get through *every* layer:
...
| Topic | Key Takeaway |
|-------|-------------|
| **Data Center** | A specialized building that keeps servers running 24/7 with power, cooling, security, and connectivity |
| **Standards** | TIA-942 (design), ASHRAE (climate), NFPA (fire safety) provide the rules |
| **Tiers** | Tier I (basic) to Tier IV (fault-tolerant) — higher tier = less downtime, more cost |
| **Power** | Three layers: utility → UPS (batteries) → generator — so power never stops |
| **Cooling** | CRAC/CRAH systems + hot/cold aisle design keep servers at 18–27°C |
| **Fire Protection** | VESDA detects smoke early; clean agents (FM-200, Novec) suppress fire without water damage |
| **Security** | Multiple layers: access cards → biometrics → mantraps → cameras |
| **Monitoring** | NOC staff + DCIM software watch everything 24/7 |
| **Disaster Recovery** | A backup site (cold/warm/hot) takes over if the primary fails; RTO = recovery speed, RPO = data loss tolerance |
| **PUE** | Total power ÷ IT power — closer to 1.0 is better |