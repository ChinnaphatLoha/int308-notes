...
export default function QuizWorkspace({ quizzes }: QuizWorkspaceProps) {
	...
	const stats = useMemo(() => {
		const answeredCount = runtimeQuestions.filter(
			(q) => answers[q.id] && !answers[q.id].skipped,
		).length;
		const correctCount = runtimeQuestions.filter(
			(q) => answers[q.id]?.choiceId === q.correctChoiceId,
		).length;
		const completion =
			runtimeQuestions.length === 0
				? 0
				: ((answeredCount +
						runtimeQuestions.filter((q) => answers[q.id]?.skipped).length) /
	...
	const selectAnswer = (choiceId: Choice['id']) => {
		setAnswers((prev) => ({
			...prev,
			[currentQuestion.id]: { choiceId, skipped: false },
		}));
	};
	const skipAnswer = () => {
		setAnswers((prev) => ({
				...
				<Stack spacing={2.5}>
						...
						<Stack spacing={2.25}>
							...
							>
								...
								<Stack
									direction='row'
									spacing={1}
									sx={{ alignItems: 'center', flexWrap: 'wrap', width: '100%', gap: 1 }}
								>
									<QuizSettingsDialog
										quiz={selectedQuiz}
										settings={settings}
                    ...
                    isXs={isXs}
                      ...
                      ml: '0 !important'
									...
									>
										<InputLabel id='quiz-select-label'>
											Question bank
										</InputLabel>
										<Select
					...
					>
								...
								<Stack spacing={2}>
									...
									>
										...
										<Button
											variant='outlined'
											color='secondary'
											onClick={() => initializeQuiz(selectedQuiz, settings)}
										>
											<RestartAltIcon />
										</Button>
									</Stack>
						...
						{reviewMode ? (
							<ReviewPanel
								...
								onReset={() => initializeQuiz(selectedQuiz, settings)}
							/>
						) : (
							<QuestionPanel
								...
								onNext={() =>
									setCurrentIndex((c) =>
										Math.min(runtimeQuestions.length - 1, c + 1),
									)
								}
